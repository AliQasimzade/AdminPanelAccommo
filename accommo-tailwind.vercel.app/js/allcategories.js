  ...
  categories.forEach(category => {
    tbody.innerHTML +=
      `
        ...
        <img src= ${category.image} alt = ${category.name}  class="w-12 h-12">
        ...
        <td class="border-b border-gray-200 dark:border-gray-900 whitespace-nowrap text-sm font-medium text-gray-700 dark:text-gray-300 px-6 py-3">
          <div class="flex items-center">
            <button type="button" onclick="toggleModal('${category._id}')" class="border mr-2 border-gray-200 hover:bg-blue-500 dark:hover:border-blue-500 hover:text-white text-gray-700 dark:text-gray-300 dark:border-gray-800 rounded-full w-8 h-8 flex justify-center items-center">
            ...
            <button type="button" onclick="deleteElement('${category._id}')" class="border mr-2 border-gray-200 hover:bg-blue-500 dark:hover:border-blue-500 hover:text-white text-gray-700 dark:text-gray-300 dark:border-gray-800 rounded-full w-8 h-8 flex justify-center items-center">
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none" class="w-5 h-5" xmlns="http://www.w3.org/2000/svg">
  ...
  if (updateName.value === findCategoryById.name &&
    ...
    try {
      ...
      const request = await fetch(`https://adminpanelback.onrender.com/api/updatecategory/${findId}`, {
        ...
        body: JSON.stringify(updateCategoryObj)
      })
