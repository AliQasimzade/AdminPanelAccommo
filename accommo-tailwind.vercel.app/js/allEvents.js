  ...
  events.forEach(item => {
    tbody.innerHTML +=
      `
        ...
        <td class="border-b border-gray-200 dark:border-gray-900 whitespace-nowrap text-sm font-regular text-gray-500 dark:text-gray-300 px-6 py-3"> <img src="${item.image}" alt="${item.name}" width="30px" height="30px"/> </td>
        <td class="border-b border-gray-200 dark:border-gray-900 whitespace-nowrap text-sm font-regular text-gray-500 dark:text-gray-300 px-6 py-3"> ${item.locationName} </td>
        <td class="border-b border-gray-200 dark:border-gray-900 whitespace-nowrap text-sm font-regular text-gray-500 dark:text-gray-300 px-6 py-3"> ${item.locationAddress} </td>
        <td class="border-b border-gray-200 dark:border-gray-900 whitespace-nowrap text-sm font-regular text-gray-500 dark:text-gray-300 px-6 py-3"> ${item.startDate} </td>
        <td class="border-b border-gray-200 dark:border-gray-900 whitespace-nowrap text-sm font-regular text-gray-500 dark:text-gray-300 px-6 py-3"> ${item.endDate} </td>
        <td class="border-b border-gray-200 dark:border-gray-900 whitespace-nowrap text-sm font-regular text-gray-500 dark:text-gray-300 px-6 py-3"> ${item.entryPrice} &#8380; </td>
        <td class="border-b border-gray-200 dark:border-gray-900 whitespace-nowrap text-sm font-regular text-gray-500 dark:text-gray-300 px-6 py-3"> ${item.contactInfo} </td>
        <td class="border-b border-gray-200 dark:border-gray-900 whitespace-nowrap text-sm font-regular text-gray-500 dark:text-gray-300 px-6 py-3"> ${item.details} </td>
        ...
        <td class="border-b border-gray-200 dark:border-gray-900 whitespace-nowrap text-sm font-medium text-gray-700 dark:text-gray-300 px-6 py-3">
          <div class="flex items-center">
            ...
            <button onclick="deleteEvent('${item._id}')" type="button" class="border mr-2 border-gray-200 hover:bg-blue-500 dark:hover:border-blue-500 hover:text-white text-gray-700 dark:text-gray-300 dark:border-gray-800 rounded-full w-8 h-8 flex justify-center items-center">
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none" class="w-5 h-5" xmlns="http://www.w3.org/2000/svg">
...
function toggleModal(id) {
  ...
  if (id) {
    ...
    if (findEventById) {
      ...
      const blob = new Blob([findEventById.image], { type: 'image/*' });
      const file = new File([blob], findEventById.image, { type: 'image/*' });
...
const updateEvent = async () => {
  try {
    ...
    if (updateName.value == '' ||
      ...
      updateDetails.value == '' ||
      getImageUrl == '') {
      alert('Please fill input or all inputs')
    } else {
      const updateEvent = {
...
async function deleteEvent(id) {
  try {
    const req = await fetch(`https://adminpanelback.onrender.com/api/deleteevent/${id}`, {
      method:"DELETE",
      headers: {
        "Content-Type":"application/json"
      }
    ...
    if (!req.ok) {
      ...
      const res = req.json()
      alert("Delete event succesfully !")
      location.reload()
    }
  } catch (err) {
    alert(err.message)
