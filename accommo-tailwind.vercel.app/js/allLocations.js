...
const getAllLocations = async () => {
    try {
        const req = await fetch('https://adminpanelback.onrender.com/api/locations')
        if (!req.ok) {
            throw new Error("Request is failed !")
        } else {
            const res = await req.json()
            locations = [...res]
            res.forEach(location => tbody.innerHTML +=
                `
                  <tr class="align-middle hover:bg-gray-50 dark:hover:bg-background">
                  
                  <td class="border-b border-gray-200 dark:border-gray-900 whitespace-nowrap text-sm font-regular text-gray-500 dark:text-gray-300 px-6 py-3"> ${location.name} </td>
                  <td class="border-b border-gray-200 dark:border-gray-900 whitespace-nowrap text-sm font-regular text-gray-500 dark:text-gray-300 px-6 py-3"> <img src="${location.image}" alt="${location.name}" width="30px" height="30px"/> </td>
                  ...
                  <td class="border-b border-gray-200 dark:border-gray-900 whitespace-nowrap text-sm font-medium text-gray-700 dark:text-gray-300 px-6 py-3">
                    <div class="flex items-center">
                      <button type="button" onclick="toggleModal('${location._id}')" class="border mr-2 border-gray-200 hover:bg-blue-500 dark:hover:border-blue-500 hover:text-white text-gray-700 dark:text-gray-300 dark:border-gray-800 rounded-full w-8 h-8 flex justify-center items-center">
                      ...
                      <button onclick="deleteLocation('${location._id}')" type="button" class="border mr-2 border-gray-200 hover:bg-blue-500 dark:hover:border-blue-500 hover:text-white text-gray-700 dark:text-gray-300 dark:border-gray-800 rounded-full w-8 h-8 flex justify-center items-center">
                        <svg width="20" height="20" viewBox="0 0 20 20" fill="none" class="w-5 h-5" xmlns="http://www.w3.org/2000/svg">
...
updateLocImage.addEventListener('change', (e) => {
...
function toggleModal(id) {
    ...
    if (id) {
        ...
        findLocationById = locations.find(location => location._id === id);
        if (findLocationById) {
            updateLocName.value = findLocationById.name;
            const blob = new Blob([findLocationById.image], { type: 'image/*' });
            const file = new File([blob], findLocationById.image, { type: 'image/*' });
            const dataTransfer = new DataTransfer();
            dataTransfer.items.add(file);
            let fileInput = document.getElementById('updateLocImage');
            fileInput.files = dataTransfer.files;
            getLocImageUrl = updateLocImage.value.split(`\C:\\fakepath\\`)[1];
            console.log(getLocImageUrl)
        }
    }
...
const updateLocation = async () => {
    try {
        if (updateLocName.value == '' || getLocImageUrl == '') {
            alert("Please fill inputs or input !")
        } else if (/^\s+$/.test(updateLocName.value)) {
            alert("Please fill location name input !")
        } else {
            const updateLocationArgs = {
                name: updateLocName.value,
                image: getLocImageUrl
            }
            const req = await fetch(`https://adminpanelback.onrender.com/api/updatelocation/${findId}`, {
                method: 'PUT',
                headers: {
                    "Content-Type": "application/json"
                },
                body: JSON.stringify(updateLocationArgs)
            })
            ...
            if (!req.ok) {
                ...
                const res = await req.json()
                alert("Updated location was successfully !")
                location.reload()
            }
        }
    } catch (err) {
...
async function deleteLocation(id) {
    try {
        const req = await fetch(`https://adminpanelback.onrender.com/api/deletelocation/${id}`, {
            method: 'DELETE',
        ...
        if (!req.ok) {
            ...
            const res = await req.json();
            alert("Deleted location was successfully !")
            location.reload()
        }
    } catch (err) {
        alert(err.message)
